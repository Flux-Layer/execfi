...
import { splitLongWords } from "@utils/index";
  ...
  const handleSubmitLine = async (value: string) => {
    ...
    if (curQuestion) {
      if (curQuestion?.key === "email") {
        ...
        console.log({ tokenSelections, value });
      ...
      try {
        ...
        if (tokens) {
          setTokenSelections(
            (tokens?.data as unknown as any[])?.map((t, i) => ({
              id: i + 1,
              ...t,
            })),
          );
          setChat([
            {
              role: "assistant",
              content: (tokens?.data as any)?.map((t: any, i: number) => {
                return `${i + 1} => address: ${t?.address}, chainId: ${t?.chainId}, name: ${t?.name}, ticker: ${t?.symbol}\n`;
              }),
            },
          ]);
          setQuestions([
