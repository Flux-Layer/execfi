...
export default function useSmartWallet() {
   ...
   useEffect(() => {
      localStorage.setItem('selectedSmartWalletIndex', selectedSmartWalletIndex.toString());
   }, [selectedSmartWalletIndex]);

   // Reset selected index if it's out of bounds
   useEffect(() => {
      if (selectedSmartWalletIndex >= smartWallets.length && smartWallets.length > 0) {
   ...
   const handleSetSelectedSmartWalletIndex = useCallback((index: number) => {
   ...
   const formatSmartWalletAddress = useCallback((address: string | undefined) => {
      if (!address) return '';
      return `${address.slice(0, 6)}...${address.slice(-4)}`;
   }, []);

   // Copy Smart Wallet address helper
   const copySmartWalletAddress = useCallback(async (address: string): Promise<boolean> => {
   ...
   return {
      ...
      smartWalletClient,
      smartAccountAddress: selectedSmartWallet?.address as `0x${string}` | undefined,
